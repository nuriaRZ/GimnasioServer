package com.gimnasio.jwtSecurity;
...
public class AutenticadorJWT {
	...
	public static int getIdSesionDesdeJWT (String jwt) {
		try {
			String stringIdSesion = Jwts.parser().setSigningKey(getGeneratedKey()).parseClaimsJws(jwt).getBody().getSubject();
			int idSesion = Integer.parseInt(stringIdSesion);
			return idSesion;
		}
		catch (Exception ex) {
			ex.printStackTrace();
	...
	public static int getIdSesionDesdeJwtIncrustadoEnRequest (HttpServletRequest request) {
		String autHeader = request.getHeader("Authorization");
		if (autHeader != null && autHeader.length() > 8) {
			String jwt = autHeader.substring(7);
			return getIdSesionDesdeJWT(jwt);
		}
		else {
			return -1; // Bandera, ha sido imposible obtener el id del usuario
		}
